...
const Sidebar = ({ open, setOpen }) => {
  ...
  return (
    <div className={`min-w-[347px] ${open ? "block" : "hidden"} lg:block max-w-[347px] h-full p-5 shadow overflow-hidden`}>
      <h3 className="text-black/70 text-base font-bold">Messages</h3>
      <SearchBar state={searchValue} setState={setSearchValue} />
      ...
      <div className="mt-5 h-[500px] overflow-y-auto w-full">
        {contacts?.length > 0 ? (
          contacts
            ?.filter(
              (item) =>
                item?.user?.id?.firstName
                  ?.toLowerCase()
                  .includes(searchValue?.toLowerCase()) ||
                item?.user?.id?.lastName
                  ?.toLowerCase()
                  .includes(searchValue?.toLowerCase())
            )
